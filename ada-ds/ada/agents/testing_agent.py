    ...
    if os.path.exists(features_path):
        ...
        if X.shape[1] > n:
            ...
            for i in range(n - X.shape[1]):
                X[f"_pad_{i}"] = 0
  NEVER raise a custom "Feature count does not match" error — always apply alignment instead.
- ALWAYS include ALL necessary imports at the top of the script. In particular:
  ...
  VarianceThreshold is in sklearn.feature_selection — NEVER import it from sklearn.preprocessing.
...
FEATURE MISMATCH FIX: If the error mentions features/shape mismatch or feature count, replace any custom error with this alignment block AFTER all preprocessing and BEFORE predict():
    features_path = model_path.replace(".pkl", "_features.pkl")
    if os.path.exists(features_path):
        saved_cols = pickle.load(open(features_path, "rb"))
        for col in saved_cols:
        ...
        if X.shape[1] > n:
            ...
            for i in range(n - X.shape[1]):
                X[f"_pad_{i}"] = 0
NEVER raise a "Feature count does not match" error — always apply the alignment above instead."""
...
def run(
    ...
    from ada.tools.runner import run_script_with_retry
